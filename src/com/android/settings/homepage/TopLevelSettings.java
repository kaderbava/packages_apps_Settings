...
import com.android.settings.utils.IconTinterUtils;
...
public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
    ...
    if (Flags.homepageRevamp()) {
        Preference aboutDevicePref = getPreferenceScreen().findPreference("top_level_animated");
        if (aboutDevicePref != null) {
            aboutDevicePref.setLayoutResource(R.layout.preference_about_device);
        }
        iteratePreferences(preference -> {
            ...
            if (prefKey.equals("top_level_wellbeing")
                ...
                || prefKey.equals("dashboard_tile_pref_com.google.android.gms.backup.component.BackupOrRestoreSettingsActivity")) {
                preference.setVisible(false);
                return;
            }
            // Show/hide animated about device tile based on setting (enabled by default)
            if (prefKey.equals("top_level_animated")) {
                boolean shouldHide = android.provider.Settings.System.getInt(
                        getContentResolver(), "show_animated_preference", 1) == 0;
                preference.setVisible(!shouldHide);
            }
        });
    }
}
