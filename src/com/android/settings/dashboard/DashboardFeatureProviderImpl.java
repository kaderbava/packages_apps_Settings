...
public class DashboardFeatureProviderImpl implements DashboardFeatureProvider {
    ...
    // Cached set of homepage preference keys defined in top_level_settings_expressive.xml
    private static Set<String> sTopLevelDefinedKeys;

    private static void ensureTopLevelDefinedKeys(Context context) {
        if (sTopLevelDefinedKeys != null) return;
        sTopLevelDefinedKeys = new HashSet<>();
        try {
            XmlResourceParser parser = context.getResources().getXml(R.xml.top_level_settings_expressive);
            ...
            while ((type = parser.next()) != XmlPullParser.END_DOCUMENT) {
                if (type == XmlPullParser.START_TAG) {
                    ...
                    if (key != null && !key.isEmpty()) {
                        sTopLevelDefinedKeys.add(key);
                    }
                }
            }
            parser.close();
        } catch (Exception e) {
            Log.w(TAG, "Failed parsing top_level_settings_expressive.xml; icon styling will fallback.", e);
        }
    }
    ...
    private void setPreferenceIcon(Preference preference, Tile tile, boolean forceRoundedIcon,
        ...
        // Force apply Settings icon styles after icon set (applies tints/backgrounds as per
        // user settings).
        try {
            ensureTopLevelDefinedKeys(mContext);
            boolean force = mContext.getResources().getBoolean(
                    R.bool.config_force_settings_icon_styles);
            if (force && (sTopLevelDefinedKeys.contains(preference.getKey())
                    || TextUtils.equals(tile.getCategory(), CategoryKey.CATEGORY_HOMEPAGE))) {
                com.android.settings.utils.IconTinterUtils.tintSinglePreferenceIcon(
                        preference, preference.getContext());
            }
        } catch (Throwable t) {
            // Be safe in case of any unexpected issues; fallback to default icon
            Log.w(TAG, "Icon style application failed for " + tile.getTitle(mContext), t);
        }
    }
